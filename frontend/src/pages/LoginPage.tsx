import { useEffect } from 'react';
import { useNavigate, useSearchParams, Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import { BarChart3 } from 'lucide-react';
import { Button } from '../components/ui/Button';
import { useAuthStore } from '../store';
import { authApi } from '../services/api';

export default function LoginPage() {
  const navigate = useNavigate();
  const [params] = useSearchParams();
  const { setAuth, isAuthenticated, authReady } = useAuthStore();

  useEffect(() => {
    if (!authReady) return;

    const token = params.get('token');
    if (token) {
      localStorage.setItem('token', token);
      authApi.me().then(({ data }) => {
        setAuth(token, data.user, data.hasGoogleAdsAccess, data.isReturningUser);
        navigate('/settings');
      }).catch(() => navigate('/'));
      return;
    }
    if (isAuthenticated) navigate('/settings');
  }, [params, setAuth, navigate, isAuthenticated, authReady]);

  const handleMockLogin = async () => {
    const { data } = await authApi.login('demo@acmeplumbing.com.au', 'Demo User');
    setAuth(data.token, data.user, false);
    navigate('/settings');
  };

  const handleGoogleLogin = () => {
    window.location.assign(authApi.googleUrl('/login', false));
  };

  if (!authReady) {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <div className="skeleton w-48 h-6 rounded" />
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-bg flex items-center justify-center relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(255,107,43,0.06),transparent_70%)]" />
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-panel border border-border rounded-2xl p-8 w-full max-w-md relative z-10 glow-orange"
      >
        <div className="flex items-center justify-center gap-2 mb-8">
          <div className="w-10 h-10 rounded-lg bg-orange/10 border border-orange/20 flex items-center justify-center">
            <BarChart3 size={24} className="text-orange" />
          </div>
          <div>
            <div className="font-bold text-white text-xl">AdAudit <span className="text-orange">Pro</span></div>
            <div className="text-[10px] text-muted uppercase tracking-wider">Google Ads Audit Engine</div>
          </div>
        </div>

        <h1 className="text-white text-2xl font-bold text-center mb-2">Welcome back</h1>
        <p className="text-muted text-sm text-center mb-8">Sign in to access your audit reports</p>

        <div className="space-y-3">
          <Button variant="secondary" className="w-full" onClick={handleGoogleLogin}>
            <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
            Continue with Google
          </Button>
          <Button className="w-full" onClick={handleMockLogin}>
            Demo Login (Mock)
          </Button>
        </div>

        <p className="text-muted text-xs text-center mt-6">
          <Link to="/" className="text-orange hover:underline">← Back to home</Link>
        </p>
      </motion.div>
    </div>
  );
}
